...
final class ExtensionTests: BaseTestCase {
    ...
    //issueのcontentとissueContentの関係を検証
    ...
    //issueのcreationDateとissueCreationDateの関係を確認
    ...
    //Tagをissueに追加した際に、issueTagの数が増えるか確認
    ...
    //Tagをissueに追加した際に、issuetagListが"My Tag"になっているか確認
    ...
    //tagのnameとtagNameの関係を確認
    func testTagNameunwrap() {
        let tag = Tag(context: managedObjectContext)
        
        tag.name = "Example Tag"
        XCTAssertEqual(tag.tagName, "Example Tag", "Change name should also change tagName")
    }
    
    //アクティブなIssueの拡張機能が機能することを確認
    ...
    func testTagSortingIsStable() {
        ...
        //FFFFFFFF で始まるように設定して、同じ名前を持つ他のタグの後にソートされるようにする
